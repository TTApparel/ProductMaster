...
class ProductMaster_Admin_Portal
{
    ...
    {
        $atts = shortcode_atts(array('id' => ''), $atts, 'productmaster_product_loop');
        $loop = $this->get_product_loop_by_id(sanitize_key($atts['id']));
        if (empty($loop)) {
            return '';
        }

        $paged = max(1, absint(get_query_var('paged') ? get_query_var('paged') : (isset($_GET['paged']) ? wp_unslash($_GET['paged']) : 1))); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        $query = new WP_Query(array('post_type' => 'product', 'post_status' => 'publish', 'posts_per_page' => (int) $loop['per_page'], 'paged' => $paged));
        $this->apply_filters_to_product_query($query);

        ob_start();
        echo '<div class="productmaster-product-loop" style="--pm-col-desktop:' . (int) $loop['columns_desktop'] . ';--pm-col-tablet:' . (int) $loop['columns_tablet'] . ';--pm-col-mobile:' . (int) $loop['columns_mobile'] . ';">';
        echo '<div class="productmaster-loop-grid">';
        while ($query->have_posts()) { $query->the_post(); $product = wc_get_product(get_the_ID()); if (!$product) { continue; }
            echo '<article class="productmaster-loop-card">';
            if ('1' === $loop['show_image']) { echo '<a href="' . esc_url(get_permalink()) . '">' . $product->get_image() . '</a>'; }
            if ('1' === $loop['show_name']) { echo '<h3><a href="' . esc_url(get_permalink()) . '">' . esc_html($product->get_name()) . '</a></h3>'; }
            if ('1' === $loop['show_price']) { echo '<div class="price">' . wp_kses_post($product->get_price_html()) . '</div>'; }
            echo '</article>';
        }
        echo '</div>';
        if ('1' === $loop['show_pagination']) { echo paginate_links(array('total' => $query->max_num_pages, 'current' => $paged)); }
        echo '</div>';
        wp_reset_postdata();
        return ob_get_clean();
    }
    ...
    {
        $loops = $this->get_saved_product_loops();
        echo '<section class="productmaster-card"><h2>' . esc_html__('Create Product Loop', 'productmaster') . '</h2><form method="post">';
        wp_nonce_field('productmaster_save_tax_filter', 'productmaster_tax_filter_nonce');
        echo '<input type="hidden" name="productmaster_action" value="add_product_loop" />';
        echo '<p><label>' . esc_html__('Loop Name', 'productmaster') . ' <input type="text" name="loop_label" class="regular-text" required></label></p>';
        echo '<p><label>Per page <input type="number" min="1" max="48" name="per_page" value="12"></label></p>';
        echo '<p><label>Desktop cols <input type="number" min="1" max="6" name="columns_desktop" value="4"></label> <label>Tablet cols <input type="number" min="1" max="4" name="columns_tablet" value="2"></label> <label>Mobile cols <input type="number" min="1" max="2" name="columns_mobile" value="1"></label></p>';
        echo '<p><label><input type="checkbox" name="show_image" value="1" checked> Show image</label> <label><input type="checkbox" name="show_name" value="1" checked> Show name</label> <label><input type="checkbox" name="show_price" value="1" checked> Show price</label> <label><input type="checkbox" name="show_pagination" value="1" checked> Show pagination</label></p>';
        submit_button(__('Save Product Loop', 'productmaster'));
        echo '</form></section>';
        echo '<section class="productmaster-card"><h2>' . esc_html__('Saved Product Loops', 'productmaster') . '</h2>';
        foreach ($loops as $loop) { echo '<p><strong>' . esc_html($loop['label']) . '</strong> <code>[productmaster_product_loop id="' . esc_attr($loop['id']) . '"]</code><form method="post" style="display:inline">'; wp_nonce_field('productmaster_save_tax_filter', 'productmaster_tax_filter_nonce'); echo '<input type="hidden" name="productmaster_action" value="delete_product_loop" /><input type="hidden" name="loop_id" value="' . esc_attr($loop['id']) . '" />'; submit_button(__('Delete', 'productmaster'), 'delete small', 'submit', false); echo '</form></p>'; }
        echo '</section>';
    }
    ...
    private function get_saved_product_loops()
    {
        ...
        return is_array($loops) ? $loops : array();
    }

    private function get_product_loop_by_id($id)
    {
        foreach ($this->get_saved_product_loops() as $loop) { if (!empty($loop['id']) && $loop['id'] === $id) { return $loop; } }
        return array();
    }
    ...
    {
        return array(
            'label' => isset($data['loop_label']) ? sanitize_text_field(wp_unslash($data['loop_label'])) : '',
            'per_page' => isset($data['per_page']) ? max(1, min(48, absint($data['per_page']))) : 12,
            'columns_desktop' => isset($data['columns_desktop']) ? max(1, min(6, absint($data['columns_desktop']))) : 4,
            'columns_tablet' => isset($data['columns_tablet']) ? max(1, min(4, absint($data['columns_tablet']))) : 2,
            'columns_mobile' => isset($data['columns_mobile']) ? max(1, min(2, absint($data['columns_mobile']))) : 1,
            'show_image' => isset($data['show_image']) ? '1' : '0',
            'show_name' => isset($data['show_name']) ? '1' : '0',
            'show_price' => isset($data['show_price']) ? '1' : '0',
            'show_pagination' => isset($data['show_pagination']) ? '1' : '0',
        );
    }
